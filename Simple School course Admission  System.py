...
firstw=Tk()
firstw.title("ADMISSION")
firstw.geometry("1600x1000+0+0")
label=Label(text="SCHOOL ADMISSION",font=("times new roman",35),bg="MediumOrchid2")
...
def second():
    ...
    def studentid():
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT id ,name, fee , branch,sur FROM basic ')
    ...
    def viewenquiry2():
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * FROM advance')
    ...
    def distroy5():
        secondw.destroy()
        window()
    mainlabel= Label(secondw,text="SCHOOL", font=("times new roman", 35), bg="MediumOrchid2")
    mainlabel.pack(side=TOP, fill=X)
    button = Button(secondw,width=13, font=("arial", 20), text="REGISTRATION", bg="MediumOrchid2", command=distroy4)
    button.place(x=10, y=480)
    enquiry = Button(secondw, width=13, font=("arial", 20), text="FEE DETAILS", bg="MediumOrchid2",command=distroy5)
    enquiry.place(x=260, y=480)
    fee_details = Button(secondw, width=13, font=("arial", 20), text="ENQUIRY", bg="MediumOrchid2",command=enquiry1)
    fee_details.place(x=500, y=480)
    viewenquiry= Button(secondw, width=13, font=("arial", 20), text="VIEW ENQUIRY", bg="MediumOrchid2",command=viewenquiry2)
    viewenquiry.place(x=740, y=480)
    viewenquiry1 = Button(secondw, width=13, font=("arial", 20), text="STUDENT ID ", bg="MediumOrchid2",command=studentid)
...
#=========================buttton==========================
    ...
    button3=Button(root,text="RESET",width=14,font=("arial",10),bg="indianred1",command= advance )
...
with conn:
    cur=conn.cursor()
    cur.execute('CREATE TABLE IF NOT EXISTS basic(id INTEGER primary key autoincrement ,name text,sur text,email, branch text,gender text, fee int, python int,java int,c int,d int)')
    cur.execute("UPDATE SQLITE_SEQUENCE SET seq = 1000 WHERE name = 'basic'")
    cur.execute('CREATE TABLE IF NOT EXISTS advance (NAME TEXT, PHONE INT ,PURPOSE TEXT,DATE)')
...
def ka():
    ...
    with conn:
        cur=conn.cursor()
        cur.execute('INSERT INTO advance(NAME,PHONE,PURPOSE,DATE)VALUES(?,?,?,?)',(NAMEE,PHONE,PURPOSE,today))
        conn.commit()
def r():
    j()
    ka()
...
def cat():
    ...
    with conn:
        cur=conn.cursor() 
        cur.execute('SELECT fee FROM basic WHERE id=?',(FE,))
...
def msg():
 ...
 if  "@" and ".com" not in entry3.get() :
    ...
    if msg>0:
     ...
     with conn:
         cur=conn.cursor()
         cur.execute('INSERT INTO basic (name,sur, email, branch, gender,fee ,python,java,c,d ) VALUES(?,?,?,?,?,?,?,?,?,?)',(NAME,SUR,EMAIL,BRANCH,GENDER,CALCULATE,PYTHON,JAVA,C,D,))
...
def search_id():
    ...
    with conn:
        cur=conn.cursor()
        cur.execute('SELECT name FROM basic WHERE id=?',(SEARCH,))
...
def search_sur():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT sur FROM basic WHERE id=?',(SEARCH,))
...
def search_email():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT email FROM basic WHERE id=?',(SEARCH,))
...
def search_branch():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT branch FROM basic WHERE id=?',(SEARCH,))
...
def search_gender():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT gender FROM basic WHERE id=?',(SEARCH,))
...
def search_course():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT python FROM basic WHERE id=?',(SEARCH,))
            for row6 in cur.fetchone():
                python.set(row6)
            cur.execute('SELECT java FROM basic WHERE id=?',(SEARCH,))
            for row7 in cur.fetchone():
                java.set(row7)
            cur.execute('SELECT c FROM basic WHERE id=?',(SEARCH,))
            for row8 in cur.fetchone():
                c.set(row8)
            cur.execute('SELECT d FROM basic WHERE id=?',(SEARCH,))
            for row9 in cur.fetchone():
                d.set(row9)
            cur.execute('SELECT fee FROM basic WHERE id=?',(SEARCH,))
...
def update():
    ...
    if box1>0:
     ...
     with conn:
         cur=conn.cursor()
         cur.execute('UPDATE basic SET name=? WHERE id=?',(NAME,SEARCH,))
         cur.execute('UPDATE basic SET sur=? WHERE id=?',(SUR,SEARCH,))
         cur.execute('UPDATE basic SET email=? WHERE id=?',(EMAIL,SEARCH,))
         cur.execute('UPDATE basic SET branch=? WHERE id=?',(BRANCH,SEARCH,))
         cur.execute('UPDATE basic SET gender=? WHERE id=?',(GENDER,SEARCH,))
         cur.execute('UPDATE basic SET fee=? WHERE id=?',(FEE,SEARCH,))
         cur.execute('UPDATE basic SET python=? WHERE id=?',(PYTHON,SEARCH,))
         cur.execute('UPDATE basic SET java=? WHERE id=?',(JAVA,SEARCH,))
         cur.execute('UPDATE basic SET c=? WHERE id=?',(C,SEARCH,))
         cur.execute('UPDATE basic SET d=? WHERE id=?',(D,SEARCH,))
         conn.commit()
...
def delete():
    ...
    if box>0:
        ...
        CALCULATE=calculate.get()
           ...
           with conn:
              cur=conn.cursor()
              cur.execute("DELETE FROM basic WHERE id=?",(SEARCH,))
              conn.commit()
              advance()
